 ...
 */
void SFile_SDT::Open(const char *filename, enum sfile_mode openmode) {
	mode = openmode;
	...
	switch (mode) {
		case SFILE_MODE_READ:
			...
			if (f == NULL) {
				...
				//return 0;
				throw SOFTLibException("Unable to open SDT file.");
			}
			Load();
			Close();
			break;
		...
		default:
			...
			throw SOFTLibException("Unrecognized option for opening SDT file.");
	}
}
...
void SFile_SDT::Close(void) {
	int i;
	if (mode == SFILE_MODE_READ) {
		if (kl == NULL) return;

		for (i = 0; i < kl->nkeys; i++) {
			delete kl->keys[i].name;
		}
...
void SFile_SDT::Load(void) {
	...
	//size_t l = ftell(f);
	//fseek(f, 0, SEEK_SET);
	sdtfile.seekg(0, ios::end);
	size_t l = sdtfile.tellg();
	sdtfile.seekg(0, ios:beg);
	...
	//kl = new struct ssdt_keylist;
	//kl->keys = NULL;
	//kl->nkeys = 0;

	int i = 0, j, m, n;
	while (ssdt_file[i]) {
		keys = realloc(kl->keys, sizeof(ssdt_key)*(kl->nkeys+1));
